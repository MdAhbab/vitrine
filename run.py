#!/usr/bin/env python3
"""
Vitrine local launcher — runs the dev stack on localhost only.
...
Everything binds to 127.0.0.1 (localhost) — nothing is exposed on the network.
...
Cloud deployment lives in a separate file: on the VM, run `python3 run_onVM.py`.
"""
...
def dispatch_cloud(_argv: list[str]) -> int:
    warn("Cloud deployment has moved out of run.py.")
    info("On the VM, deploy the whole stack with:  python3 run_onVM.py")
    info("run.py now only runs the local, localhost-only dev stack.")
    return 2
